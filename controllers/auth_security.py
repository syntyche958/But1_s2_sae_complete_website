...
def auth_signup_post():
    ...
    sql = " SELECT * FROM utilisateur WHERE login = %s OR email = %s;  "  #verifier que qqn n'a pas le meme login/mot de passe   select * from utilisateur where login = %s
    ...
    sql = """  INSERT INTO utilisateur (login, email, password, role) VALUES (%s, %s, %s, %s);  """ # insert login email password role   INSERT INTO utilisateur (login,email,password,role) VALUES (%s,%s,%s,%s)
    mycursor.execute(sql, tuple_insert)
    get_db().commit()
    sql = """  SELECT last_insert_id() AS last_insert_id;  """ #  select lasii insert id as last insert id  SELECT last_insert_id() AS last_insert_id
